...
def GenerateAndUploadAktliste(dt_AktIndex, Sagsnummer, DokumentlisteDatoString,SharePointURL, Overmappe, Undermappe, tenant, client_id, thumbprint, cert_path ):
    ...
    workbook.save(AktlisteFileName)
    ...
    df = pd.read_excel(AktlisteFileName)
    ...
    def first_page(canvas, doc):
        ...
        canvas.drawImage("aak.jpg", image_x, image_y, width=image_width, height=image_height)
        ...
        # Add Date
        date_string = f"Dato for aktindsigt: {DokumentlisteDatoString}"
    ...
    # PDF document setup
    doc = SimpleDocTemplate(PDFAktlisteFilnavn, pagesize=landscape(A4),
    ...
    upload_file_to_sharepoint(
        ...
        Overmappe=Overmappe,
        Undermappe=Undermappe,
        file_path=PDFAktlisteFilnavn,
        ctx = ctx
    )

    try:
        os.remove(AktlisteFileName)
        os.remove(PDFAktlisteFilnavn)
